...
| Anonymous crash reporting | 不阻塞首個Closed Testing candidate；repository已有default-off、local-only foundation，但candidate不得宣稱已上傳或被provider收集 | repository已具opt-in notice、固定去識別化schema、bounded queue與opt-out purge；public artifact仍需外部provider/receipt gate，禁止token、姓名、通知內容、payload | TestFlight若未啟用upload，beta notes明示；repository local queue不等於provider evidence | required；同Android public且須重核App Privacy | repository redaction/queue tests + 未來synthetic crash receipt；不保存原始error、stack、敏感event/body或裝置識別碼 |
