...
| 出席報告的 Lineup 背號 | Mobile report 只有 `person_id`、`display_name`、`reply`，因此不能顯示既有 Member 背號 | 對既有可見賽事與 Officer/Admin 提供 nullable `member_number`；不暴露 `member_id` | TASK-156：API/privacy/authorization；不需 schema |
