...
- Review fix commit：`42ea8dd138d456a03e35ba1b802f0f05675625fd`
...
- 每個 stage 與 total 均 clamp 至 source-fixed `0..300000` 毫秒；這只限制異常 clock jump 的診斷值，不是 request timeout。
...
```text
Web Portal tests: 109 tests - OK (skipped=2)
compileall: OK
Python 3.10 AST grammar: 21 files - OK
git diff --check: OK
```

新增 deterministic clock、clock 倒退／極端正向 jump 上下限、未知／重複 stage、clock fault、非數字 clock、logger fault、成功 route 單筆 bounded log、敏感 sentinel 與 query/model/analyzer 呼叫次數測試。
