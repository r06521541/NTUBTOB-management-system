...
- 無效 state 會清除目前 Web Portal session，顯示安全重試頁；重試連結只會進入 `/line/login` 建立全新 nonce/state，不會重用舊 code/state。
...
- 確認 invalid-state 兩條路徑都清除 session、在任何外部或 DB 呼叫前停止，且重試產生 fresh transaction。
