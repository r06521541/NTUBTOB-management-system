...
- 套用至既有 Game/賽程、Game detail、roster、attendance、Game day 等共用 portal layout；首頁僅沿用共用 tokens，未擴張功能。交通／裝備分工不屬本次範圍。
...
- Web Portal unittest：132 passed、2 skipped；另執行 py_compile、git diff --check。未宣稱實際瀏覽器像素視覺效果。
