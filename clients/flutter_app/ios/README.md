...
Hosted CI additionally owns one unsigned archive vector (TASK-197):
...
the iOS source and native Apple bridge into an unsigned `.xcarchive` using
`flutter build ipa --release --no-codesign`. IPA export is skipped. This archive
cannot be uploaded, installed, or treated as TestFlight/signing evidence. Production and
...
The hosted macOS archive gate executes normal Xcode build phases with the unsigned
contract vector, and checks archive shape, app bundle identity and version/build.
It is isolated from the separate fictional cross-process codesign rehearsal: no
key is imported into this job, and no archive is uploaded as a workflow artifact.
