...
expect_status 0 'hosted staging TestFlight compile-only contract' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Release \
  DART_DEFINES="$real_staging_defines" \
  IOS_RELEASE_CONTRACT_VERSION=1 \
  IOS_DISTRIBUTION_CHANNEL=testflight \
  IOS_TESTFLIGHT_CONTRACT_TEST=YES \
  IOS_EXTERNAL_SIGNING_READY=NO \
  CODE_SIGNING_ALLOWED=NO \
  PRODUCT_BUNDLE_IDENTIFIER=tw.org.ntubtob.portal \
  FLUTTER_BUILD_NAME=0.1.0 \
  FLUTTER_BUILD_NUMBER=1 \
  APPLE_SIGN_IN_REPOSITORY_STATUS=not_implemented \
  APPLE_PROVIDER_CONFIGURED_EXTERNALLY=NO

expect_status 2 'compile-only contract cannot claim signing readiness' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Release \
  DART_DEFINES="$real_staging_defines" \
  IOS_RELEASE_CONTRACT_VERSION=1 \
...
expect_status 2 'compile-only contract cannot claim provider readiness' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Release \
  DART_DEFINES="$real_staging_defines" \
  IOS_RELEASE_CONTRACT_VERSION=1 \
...
expect_status 2 'compile-only contract cannot select production' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Release \
...
expect_status 2 'compile-only contract cannot use a debug configuration' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Debug \
  DART_DEFINES="$real_staging_defines" \
  IOS_RELEASE_CONTRACT_VERSION=1 \
...
expect_status 2 'compile-only contract cannot receive signing metadata' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Release \
  DART_DEFINES="$real_staging_defines" \
  IOS_RELEASE_CONTRACT_VERSION=1 \
...
expect_status 2 'compile-only contract cannot bind entitlements' \
  SRCROOT="$fixture_root" \
  CONFIGURATION=Release \
  DART_DEFINES="$real_staging_defines" \
  IOS_RELEASE_CONTRACT_VERSION=1 \
