...
from sqlalchemy import DateTime, ForeignKey, Integer, SmallInteger, String, and_, update
...
class GameAttendanceReply(Base):
    ...
    user_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("ntubtob.line_users.id")
    )
    member_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("ntubtob.members.id")
    )
    person_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("ntubtob.people.id")
    )
    ...
    def search_by_member_id(cls, member_id: int) -> list["GameAttendanceReply"]:
        with Session(engine) as session:
            results = (
                session.query(GameAttendanceReply)
                .filter(and_(GameAttendanceReply.member_id == member_id))
                .all()
            )
    ...
    def search_by_game_id(cls, game_id: int) -> list["GameAttendanceReply"]:
        with Session(engine) as session:
            results = (
                session.query(GameAttendanceReply)
                .filter(and_(GameAttendanceReply.game_id == game_id))
                .all()
            )
    ...
    @classmethod
    def search_single_game_reply_of_member(
        cls, game_id: int, member_id: int
    ) -> list["GameAttendanceReply"]:
        with Session(engine) as session:
            results = (
                session.query(GameAttendanceReply)
                .filter(
