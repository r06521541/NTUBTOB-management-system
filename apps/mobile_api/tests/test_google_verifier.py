...
class GoogleIdTokenVerifierTest(unittest.TestCase):
    def test_verified_subject_requires_google_issuer_allowlisted_audience_and_expiry(
        self,
    ):
        ...
        self.assertEqual(
            verified.audience, "approved-client.apps.googleusercontent.com"
        )
    ...
    def test_optional_web_oidc_nonce_must_match_exactly(self):
        claims = {
            "iss": "https://accounts.google.com",
            "aud": "approved-client.apps.googleusercontent.com",
            "sub": "google-stable-subject",
        ...
        verifier = GoogleIdTokenVerifier(
            lambda _token: claims, audiences=(claims["aud"],)
        )
        self.assertEqual(
            verifier.verify("raw", "", "expected-nonce", NOW).nonce,
            "expected-nonce",
        )
        with self.assertRaises(AuthenticationError):
            verifier.verify("raw", "", "wrong-nonce", NOW)
