...
class BrandUiContractTest(unittest.TestCase):
    ...
    def test_every_portal_entry_loads_shared_tokens_in_safe_order(self):
        ...
        production_base = (TEMPLATES_DIR / "_portal_base.html").read_text(
            encoding="utf-8"
        )
        self.assertIn("filename='brand.css'", production_base)
        self.assertIn("filename='production_portal.css'", production_base)
        self.assertLess(
            production_base.index("filename='brand.css'"),
            production_base.index("filename='production_portal.css'"),
        )
    ...
    def test_member_navigation_keeps_home_visible_with_core_links(self):
        ...
        self.assertIn("url_for('events')", navigation)
